...
import {
  Search, Filter, Bookmark, TrendingUp, Clock, ExternalLink, Moon, Sun, Building2, BarChart3, Calendar,
  Sparkles, RefreshCw, AlertCircle, Heart, Archive, ArchiveRestore, Trash2, Download, Upload, Database,
  Cloud, CloudOff, Settings, X, Mail, RotateCw
} from 'lucide-react';
...
const App = () => {
  ...
  // NEW: Gist-first boot flags + in-place merge reference
  const [hasLoadedGist, setHasLoadedGist] = useState(false);
  ...
  const RSS_FEEDS = (customFeeds.length > 0 ? customFeeds : DEFAULT_RSS_FEEDS).filter(f => f.enabled);
  ...
  // --- Boot: prefer Gist snapshot (prevents Recent count reverting) ---
  useEffect(() => {
    ...
    (async () => {
      if (savedToken && savedGistId) {
        await loadFromGist(true); // silent
      } else {
        const cached = JSON.parse(localStorage.getItem('cachedArticles') || '[]');
        const deleted = new Set(JSON.parse(localStorage.getItem('deletedArticles') || '[]'));
        if (cached.length) setArticles(cached.filter(a => !deleted.has(a.id)));
      }
      fetchViewCount();
    })();
  }, []);

  // --- Categorize/keywords helpers ---
  ...
  // UPDATED: loadFromGist(silent) — sets state immediately, no reload
  const loadFromGist = async (silent = false) => {
    ...
    try {
      ...
      const deleted = new Set(JSON.parse(localStorage.getItem('deletedArticles') || '[]'));
      const fromGist = (data.articles || []).filter(a => !deleted.has(a.id));
      const archivedSeparate = (data.archivedArticles || []).filter(a => !deleted.has(a.id));

      // Prefer archived copies for archived items
      const merged = mergeById(
        archivedSeparate.map(a => ({ ...a, archived: true })),
        fromGist
      );

      // Add local manual items not in snapshot
      const localManual = JSON.parse(localStorage.getItem('manualArticles') || '[]')
        .filter(a => !deleted.has(a.id));

      const finalArticles = mergeById(localManual, merged)
        .sort((a, b) => {
  ...
  const archiveArticle = (articleId) => {
    ...
    const articleToArchive = articlesRef.current.find(a => a.id === articleId);
  ...
  const exportData = (type = 'all') => {
    ...
    switch (type) {
      case 'archived':
        ...
        break;
      case 'saved':
        dataToExport = { ...exportData, articles: articlesRef.current.filter(a => savedArticles.includes(a.id)) };
  ...
  // --- RSS Fetcher (MERGES into Gist snapshot; never overwrites) ---
  useEffect(() => {
    const fetchArticles = async () => {
      ...
      try {
        ...
        const deletedArticles = JSON.parse(localStorage.getItem('deletedArticles') || '[]');
        const deletedSet = new Set(deletedArticles);

        // START WITH current state if we already loaded from Gist
        const startWith = hasLoadedGist ? articlesRef.current : [];

        const newFeedStatus = {};
        const allArticles = [];
        // fetch each feed sequentially to respect rate limits (can parallelize if you like)
            ...
            if (data.status === 'ok' && data.items) {
              const processed = data.items
                ...
                .map((item) => {
                  ...
                  if (deletedSet.has(stableId)) return null;
                  return {
                    id: stableId,
                    title,
                    source: feed.source,
              ...
              newFeedStatus[feed.source] = { status: 'success', count: processed.length, totalFetched: data.items.length };
            }
          } catch (feedError) {
        ...
        // bring in manual local articles
        const manualArticles = JSON.parse(localStorage.getItem('manualArticles') || '[]')
          .filter(a => !deletedSet.has(a.id));

        let combinedArticles = mergeById([...manualArticles, ...allArticles], startWith);

        // keep archived items from local storage that weren’t in the fetch
        const archivedLocal = JSON.parse(localStorage.getItem('archivedArticles') || '[]');
        const archivedExtras = archivedLocal
          .filter(a => !deletedSet.has(a.id) && !combinedArticles.find(x => x.id === a.id))
          .map(a => ({ ...a, archived: true }));

        combinedArticles = mergeById(archivedExtras, combinedArticles);
    ...
    fetchArticles();
    const interval = setInterval(fetchArticles, 30 * 60 * 1000); // every 30 minutes
    return () => clearInterval(interval);
  }, [hasLoadedGist]);

  // --- Load saved list on boot ---
  useEffect(() => {
  ...
  const filteredArticles = displayArticles.filter(article => {
    ...
    const isNewToday = (() => {
      ...
      const ad = new Date(article.date); ad.setHours(0, 0, 0, 0);
    ...
    if (activeTab === 'archive') return new Date(b.date) - new Date(a.date);
    return sortBy === 'date' ? new Date(b.date) - new Date(a.date) : (b.trending - a.trending);
  });
  ...
  const getArticlesAddedToday = () => {
    ...
    return articles.filter(a => {
      ...
      return ad.getTime() === today.getTime() && !a.archived;
    }).length;
  };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 z-50 animate-fadeIn" onClick={() => setShowFeedManager(false)}>
          <div className={'max-w-4xl w-full rounded-2xl sm:rounded-3xl p-4 sm:p-8 shadow-2xl animate-slideUp max-h-[90vh] overflow-y-auto ' + (darkMode ? 'bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700' : 'bg-white')} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row gap-2 sm:gap-3">
              ...
              }} className="px-4 sm:px-6 py-2 sm:py-3 rounded-full bg-gradient-to-r from-green-500 to-emerald-500 text-white hover:from-green-600 hover:to-emerald-600 transition-all hover:scale-105 shadow-lg text-sm sm:text-base">+ Add Feed</button>

              <button onClick={() => {
                if (!confirm('Reset to default feeds?')) return;
                setCustomFeeds([]); localStorage.removeItem('customFeeds');
                if (confirm('✅ Reset! Refresh now?')) window.location.reload();
              }} className={'px-4 sm:px-6 py-2 sm:py-3 rounded-full transition-all hover:scale-105 text-sm sm:text-base ' + (darkMode ? 'bg-orange-500/10 text-orange-400 hover:bg-orange-500/20' : 'bg-orange-50 text-orange-700')}>Reset</button>
            </div>
              ...
              {(customFeeds.length > 0 ? customFeeds : DEFAULT_RSS_FEEDS).map((feed, index) => (
                <div key={index} className={'p-3 sm:p-5 rounded-xl sm:rounded-2xl flex items-center justify-between transition-all hover:scale-[1.02] ' + (darkMode ? 'bg-gray-800/50 backdrop-blur-sm' : 'bg-gray-50')}>
                  ...
                  <div className="flex gap-1 sm:gap-2 flex-shrink-0">
                    ...
                    }} className={'px-2 sm:px-4 py-1 sm:py-2 rounded-full text-xs transition-all hover:scale-105 ' + ((customFeeds.length > 0 ? customFeeds[index].enabled : DEFAULT_RSS_FEEDS[index].enabled) ? 'bg-green-500/20 text-green-400' : 'bg-gray-600 text-gray-400')}>
                      {(customFeeds.length > 0 ? customFeeds[index].enabled : DEFAULT_RSS_FEEDS[index].enabled) ? 'On' : 'Off'}
                    </button>
                    <button onClick={() => {
                      const feedsToUpdate = customFeeds.length > 0 ? [...customFeeds] : [...DEFAULT_RSS_FEEDS];
                      if (!confirm(`Delete feed "${feedsToUpdate[index].source}"?`)) return;
                      feedsToUpdate.splice(index, 1);
                      setCustomFeeds(feedsToUpdate);
                      localStorage.setItem('customFeeds', JSON.stringify(feedsToUpdate));
                      if (confirm('Feed deleted! Refresh now?')) window.location.reload();
                    }} className="p-1 sm:p-2 rounded-full bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all">
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
          ...
          <div className="flex items-center justify-between py-2 sm:py-4">
            <div className="flex items-center gap-2">
              ...
              {/* Page reload moved next to day/night with a different icon */}
              <button onClick={() => window.location.reload()} className={'p-1.5 sm:p-2.5 rounded-full transition-all hover:scale-110 ' + (darkMode ? 'bg-gray-900 hover:bg-gray-800 text-green-400' : 'bg-gray-100 hover:bg-gray-200 text-green-600')} title="Reload page">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              {/* Cloud status (blue = connected/syncing with GitHub) */}
              <button onClick={() => setShowGistSettings(true)} className={'p-1.5 sm:p-2.5 rounded-full relative transition-all hover:scale-110 ' + (darkMode ? 'bg-gray-900 hover:bg-gray-800' : 'bg-gray-100 hover:bg-gray-200') + ' ' + (gistStatus === 'connected' ? 'text-blue-400' : 'text-gray-400')}>
              ...
              <div className="relative">
                ...
                {showExportMenu && (
                  <div className={'absolute right-0 mt-2 w-48 sm:w-56 rounded-xl sm:rounded-2xl shadow-2xl p-2 z-50 animate-slideUp ' + (darkMode ? 'bg-gray-900 border border-gray-800' : 'bg-white')}>
                    ...
                    {gistId && (
                      <button onClick={() => { loadFromGist(false); setShowExportMenu(false); }} className={'w-full flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-left transition-all hover:scale-[1.02] ' + (darkMode ? 'hover:bg-gray-800 text-gray-300' : 'hover:bg-gray-100')}>
                        <Cloud className="w-3 h-3 sm:w-4 sm:h-4" />
                        <span className="text-xs sm:text-sm">Sync from GitHub</span>
                      </button>
                    )}
                  </div>
                )}
              ...
              {/* Toggle charts / settings / feed manager */}
              ...
              <button onClick={() => setShowFeedManager(true)} className={'p-1.5 sm:p-2.5 rounded-full transition-all hover:scale-110 ' + (darkMode ? 'bg-gray-900 hover:bg-gray-800 text-purple-400' : 'bg-gray-100 hover:bg-gray-200 text-purple-600')}>
            ...
            <div className={'mb-4 sm:mb-6 p-3 sm:p-6 rounded-2xl sm:rounded-3xl backdrop-blur-sm ' + (darkMode ? 'bg-gray-900/50 border border-gray-800' : 'bg-gray-50 border border-gray-200')}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
                ...
                <select value={selectedSource} onChange={(e) => setSelectedSource(e.target.value)} className={'w-full px-3 sm:px-4 py-2 sm:py-3 rounded-xl sm:rounded-2xl transition-all focus:ring-2 focus:ring-purple-500 text-sm sm:text-base ' + (darkMode ? 'bg-gray-800 text-white border border-gray-700' : 'bg-white border border-gray-200')}>
                  {sources.map(s => (s === '---' ? <option key="separator" disabled>────────────</option> : <option key={s} value={s}>{s === 'all' ? 'All Sources' : s}</option>))}
                </select>
